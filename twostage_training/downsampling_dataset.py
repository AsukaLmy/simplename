...
from optimized_dataset import OptimizedJRDBInteractionDataset, get_optimized_data_loaders
...
class EpochDownsampleSampler(Sampler):
    ...
    def __init__(self, dataset, samples_per_epoch=10000, balance_classes=True):
        ...
        print(f"Dataset has {len(self.positive_indices)} positive and {len(self.negative_indices)} negative samples")
...
class DownsamplingOptimizedJRDBDataset(OptimizedJRDBInteractionDataset):
    """
    Extension of OptimizedJRDBInteractionDataset with downsampling support
    """
...
def get_downsampling_data_loaders(data_path, batch_size=16, num_workers=4, image_size=(224, 224), 
    ...
    """
    Create data loaders with epoch-based downsampling for all datasets
    ...
    Args:
        ...
        stage2_balance_strategy: Strategy for stage 2 balancing
        use_group_sampling: Whether to use group-based positive sampling
        max_group_samples_ratio: Maximum samples per group as ratio of group size
    ...
    train_dataset = DownsamplingOptimizedJRDBDataset(
    ...
    val_dataset = DownsamplingOptimizedJRDBDataset(
        data_path, split='val', transform=val_test_transform, 
        image_size=image_size, use_pose=use_pose,
        distance_multiplier=distance_multiplier,
        max_negatives_per_frame=max_negatives_per_frame,
        stage1_balance_ratio=stage1_balance_ratio,
        stage2_balance_strategy='none',  # No balancing for validation
        use_group_sampling=use_group_sampling,
        max_group_samples_ratio=max_group_samples_ratio
    )
    
    test_dataset = DownsamplingOptimizedJRDBDataset(
        data_path, split='test', transform=val_test_transform, 
        image_size=image_size, use_pose=use_pose,
        distance_multiplier=distance_multiplier,
        max_negatives_per_frame=max_negatives_per_frame,
        stage1_balance_ratio=stage1_balance_ratio,
        stage2_balance_strategy='none',  # No balancing for test
        use_group_sampling=use_group_sampling,
        max_group_samples_ratio=max_group_samples_ratio
    )
    ...
    interaction_labels = getattr(train_dataset, 'interaction_labels', 
                                ['walking_together', 'standing_together', 'conversation', 'sitting_together', 'others'])
    
    print(f"Downsampling Data Loaders Created:")
...
# Test function for the downsampling dataset
def test_downsampling_dataset():
    """Test the downsampling functionality"""
    print("Testing Downsampling Dataset...")
    ...
    train_loader, val_loader, test_loader, interaction_labels = get_downsampling_data_loaders(
        ...
        train_samples_per_epoch=1000,  # Small number for testing
    ...
    print(f"\nTesting epoch sampling (first 2 batches per epoch):")
    for epoch in range(3):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx < 2:  # Print first 2 batches
                print(f"  Batch {batch_idx}: {batch_pos} positive, {batch_neg} negative samples")
            
            if batch_count >= 10:  # Sample first 10 batches for speed
                break
        
        print(f"  First 10 batches total: {pos_count} positive, {neg_count} negative")


if __name__ == '__main__':
    test_downsampling_dataset()