...
class DualPersonStage1DownsamplingTrainer:
    """
    Trainer for Stage 1 (binary interaction detection) using dual-person architecture with downsampling
    ...
    def __init__(self, config):
        ...
        # Create optimizer - only for stage1, backbone and fusion parameters
        trainable_params = []
        # Add backbone parameters
        if config.shared_backbone:
            trainable_params.extend(self.model.backbone_A.parameters())
        else:
            trainable_params.extend(self.model.backbone_A.parameters())
            trainable_params.extend(self.model.backbone_B.parameters())
        
        # Add stage1 classifier parameters
        trainable_params.extend(self.model.stage1_classifier.parameters())
        
        # Add attention parameters if using attention fusion
        if config.fusion_method == 'attention' and hasattr(self.model, 'attention_net'):
            trainable_params.extend(self.model.attention_net.parameters())
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            outputs = self.model(person_A_images, person_B_images, stage='stage1')
    ...
    def train(self, train_loader, val_loader):
        ...
        print(f"Starting Dual-Person Stage 1 downsampling training for {self.config.epochs} epochs...")
    ...
    def save_training_stats(self, total_time, avg_epoch_time, total_samples):
        ...
        stats = {
            'experiment_info': {
                'training_type': 'dual_person_stage1_downsampling',
                'fusion_method': self.config.fusion_method,
                'shared_backbone': self.config.shared_backbone,
                'samples_per_epoch': self.config.train_samples_per_epoch,
    ...
    def generate_final_report(self, val_targets, val_preds, val_probs):
        ...
        print("\nGenerating Dual-Person Stage 1 downsampling evaluation report...")
        ...
        with open(os.path.join(self.save_dir, 'evaluation_report.txt'), 'w') as f:
            f.write("Dual-Person Stage 1 Binary Classification Report (Downsampling)\n")
            f.write("=" * 70 + "\n\n")
    ...
    def plot_confusion_matrix(self, targets, predictions, class_names):
        ...
        plt.title(f'Dual-Person Stage 1 Confusion Matrix\n(Fusion: {self.config.fusion_method}, Downsampling: {self.config.train_samples_per_epoch})')
    ...
    def plot_probability_distributions(self, targets, probabilities):
        ...
        plt.suptitle(f'Probability Distributions\n(Fusion: {self.config.fusion_method}, Downsampling: {self.config.train_samples_per_epoch})')
        plt.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description='Dual-Person Stage 1 Training with Downsampling')
...
def main():
    ...
    print("="*80)
    print("Dual-Person Stage 1 Training with Downsampling")
    print("="*80)
    ...
    # Create trainer
    trainer = DualPersonStage1DownsamplingTrainer(args)
    
    # Start training
    print(f"\nStarting training...")
    trainer.train(train_loader, val_loader)
    
    print("\nDual-Person Stage 1 downsampling training completed!")
